...
import (
	...
	"github.com/iamgoroot/backline/plugin/documentation/swaggerui/views"
)
...
func (p Plugin) Setup(_ context.Context, deps core.Dependencies) error {
	deps.Router().StaticFS("swagger-ui", staticFilesFS)

	deps.Router().GET("swagger-ui/view/by-fullname/:fullname", func(reqCtx echo.Context) error {
	...
	deps.Router().GET("swagger-ui/render/by-fullname/:fullname", handlers{Dependencies: deps}.entityTabHandler)
	// mount raw definition endpoint for reading spec
	deps.Router().GET(
		"swagger-ui/spec/by-fullname/:fullname",
